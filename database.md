...
CREATE UNIQUE INDEX PLANT_ID_UNIQUE ON PLANT_ITEMS
(PLANT_ID)
LOGGING
TABLESPACE EPAS
);
...
CREATE UNIQUE INDEX PLANT_ITEMS_PLANT_ID_UK ON PLANT_ITEMS
(LOWER("PLANT_ID"))
LOGGING
TABLESPACE EPAS
);
...
ALTER TABLE PLANT_ITEMS ADD (
  ...
  CHECK (circuit_voltage in ('120V', '208V', '277V','360VDC','480V','4.16kV','13kV'))
  ENABLE VALIDATE
...
CREATE INDEX PLAN_ITEM_PLAN_PAREN_ID_INDE ON PLANT_ITEMS
(PLANT_PARENT_ID)
LOGGING
TABLESPACE EPAS
);

CREATE OR REPLACE package EPAS_DEV.plant_item_pkg as
  type plant_items_type is table of plant_items%ROWTYPE index by binary_integer;
  plant_delete_tab plant_items_type;
end;
/

CREATE OR REPLACE TRIGGER plant_items_adr after delete on plant_items
for each row
declare
...
begin
  select *
  into   plant_item_pkg.plant_delete_tab(plant_item_pkg.plant_delete_tab.COUNT+1)
  from   plant_items
  where  id = :old.id;
end;
/
...
CREATE OR REPLACE TRIGGER plant_items_ads after delete on plant_items
begin
  for i in 1..plant_item_pkg.plant_delete_tab.COUNT loop
    insert into deleted_plant_items values plant_item_pkg.plant_delete_tab(i);
  end loop;
end;
/


CREATE OR REPLACE TRIGGER plant_items_bds before delete on plant_items
begin
  plant_item_pkg.plant_delete_tab.delete;
end;
/


DROP SEQUENCE PLANT_ITEMS_ID_SEQ;
CREATE SEQUENCE PLANT_ITEMS_ID_SEQ START WITH 287821;
...
DROP SEQUENCE ISOLATION_POINTS_ID_SEQ;

CREATE SEQUENCE ISOLATION_POINTS_ID_SEQ START WITH 1641;
...
```
### Deleted Plant Items Table

```sql
CREATE TABLE DELETED_PLANT_ITEMS
(
  ID                           NUMBER(19)       NOT NULL,
  CREATED_AT                   TIMESTAMP(6),
  UPDATED_AT                   TIMESTAMP(6),
...
CREATE OR REPLACE TRIGGER deleted_plant_items_bis before insert on deleted_plant_items
...
GRANT DELETE, INSERT, SELECT, UPDATE ON DELETED_PLANT_ITEMS TO ATLIS7_OWNER;
GRANT DELETE, INSERT, SELECT, UPDATE ON DELETED_PLANT_ITEMS TO HCO_OWNER;
GRANT DELETE, INSERT, SELECT, UPDATE ON DELETED_PLANT_ITEMS TO MIS_MAXIMO;
